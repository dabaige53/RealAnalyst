...
- registry.db 是运行层：服务执行稳定性，通过 `metadata sync-registry` 从已校验 dataset YAML 受控同步。
...
5. 需要取数前运行 `metadata status`，确认 runtime registry 和 export-ready 状态。
6. `RA:analysis-run` 在执行阶段通过 `query_registry` 读取运行层信息。
7. 报告生成后进入 `RA:report-verify`，重点检查推断口径、review 标记和结果可复核性。
8. 需要跨系统交换时，走 OSI export；这是交换路径，不是分析主路径。
