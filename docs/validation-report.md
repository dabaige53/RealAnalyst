...
3. 补齐共享日志模块 `lib/log_utils.py`，避免 `RA:data-profile` 与 `RA:report-verify` 导入失败。
...
8. 更新 `.codex-plugin/plugin.json`、根 README、skills README、runtime 文档和 sync 文档，统一使用 `RA:data-export`。
...
| `skills/metadata/scripts/metadata.py context --dataset-id demo.retail.orders --metric total_revenue` | 通过 |
| `skills/data-profile/scripts/profiling_with_meta.py` 使用 demo CSV 冒烟运行 | 通过 |
| `RA:data-export` Tableau / DuckDB CLI `--help` | 通过 |
| `RA:report-verify` CLI `--help` | 通过 |
...
当前架构整体合理：metadata 作为事实源，runtime registry 作为可执行 source 索引，`RA:data-export` 负责受控取数，`RA:data-profile` 负责数据画像，`RA:analysis-run` / `RA:report` / `RA:report-verify` 负责分析和报告闭环。合并后入口更清晰，Codex 不再需要在两个导出 skill 间选择。
...
- 一个包含异常数据的 demo CSV，用于验证 `RA:report-verify` 和 profile 信号。
