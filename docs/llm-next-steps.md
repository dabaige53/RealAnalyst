...
```text
RealAnalyst 已经装好，并且只在当前项目启用。当前项目仍然保持干净，没有创建 metadata、runtime、jobs、logs，也没有写入 demo。下一步我们先确认你要从哪里抽取元数据，再决定是否需要保存到项目里。
```
...
- 不要在安装检查阶段创建 `metadata/`、`runtime/`、`jobs/`、`logs/`。
...
- 确认安装没有创建 `metadata/`、`runtime/`、`jobs/`、`logs/`。
