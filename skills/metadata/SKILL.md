...
- **index 完成**：生成了多少条 JSONL 记录，search.db 是否创建。下一步：用 `/skill RA:metadata-search` 验证索引可用，再运行 `metadata context`。
...
- **profile-review 完成**：输出了哪些完整性缺口（应补指标 / 待人工确认 / 不建议注册为指标）。下一步：按建议补充 YAML，然后 `metadata validate --completeness`。
