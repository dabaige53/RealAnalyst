...
**不要**把这些 YAML 复制到 skill 目录中；如需读取，优先使用 `RA:reference-lookup` 的查询脚本按需检索，避免 `read` 全量大文件。

### 0.1 读取数据源元数据配置

当确定数据源后，**必须先读取**对应 dataset 的 metadata context pack，再用 context 中的 `dataset.runtime_source_id` 查询运行 registry。
