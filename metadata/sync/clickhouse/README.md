...
这里保存 ClickHouse connector 发现或同步后的 catalog 快照示例。
这些快照只帮助整理 RealAnalyst metadata，不是业务定义真源。

## 推荐流程

```bash
python3 skills/metadata/adapters/clickhouse/scripts/discover_catalog.py \
  --source-id <dataset_id> \
  --database <database> \
  --table <table> \
  --connection-ref <ENV_JSON_REF> \
  --output metadata/sync/clickhouse/<dataset_id>.catalog.json
```
